...
> 面向听障骑手的 360° 多模态安全辅助系统。一台 X4 Air 同时提供 360° 画面、Ambisonic 空间音频和 IMU 姿态（三者共用同一时钟基准与同一坐标系），边缘 HUD 与左右触觉把“什么声音、从哪来、是否危险”在一秒内告诉用户。
...
**收音模式对照测试**：官方描述「立体声 = 无降噪处理、高度还原现场」，而「智能降风噪」是非线性处理，**很可能破坏声道间相位——而相位正是方位估计的命根子**。两种模式都要测。
