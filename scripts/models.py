...
def main():
    ...
    if not (root/"yamnet/saved_model.pb").exists():
        ...
        archive=root/"yamnet.tar.gz";archive.write_bytes(response.content)
        with tarfile.open(archive) as tar:tar.extractall(root/"yamnet",filter="data")
    if not (root/"yolov8n.onnx").exists():
        from ultralytics import YOLO
        YOLO(str(root/"yolov8n.pt")).export(format="onnx",opset=17,simplify=False,imgsz=640)
    manifest={"yamnet_source":url,"yolo_source":"https://github.com/ultralytics/assets", "sha256":{}}
    for p in root.rglob("*"):
        if p.is_file() and p.name!="manifest.json":manifest["sha256"][str(p.relative_to(root))]=hashlib.sha256(p.read_bytes()).hexdigest()
