...
GROUPS = {
    "horn": ["Vehicle horn, car horn, honking", "Air horn, truck horn", "Honking"],
    "siren": [
        "Siren",
        "Civil defense siren",
        "Police car (siren)",
...
class AudioWindow:
    ...
    def append(self, pcm, rate, source, timestamp):
        if source != self.source or (
            self.last_time is not None and timestamp - self.last_time > 0.35
        ):
            self.clear()
        self.source = source
...
def intensity(pcm, mapping):
    ...
    ix = float(np.mean(w * x)) * mapping["sx"]
    iy = float(np.mean(w * y)) * mapping["sy"]
