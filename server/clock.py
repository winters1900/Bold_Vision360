...
class CameraClock:
    ...
    def align(self, kind, raw, receipt):
        ...
        if elapsed >= 0.5 and raw > baseline:
            rate = (raw - baseline) / elapsed
            options = ((1e-6, "microseconds", 1e6), (1e-3, "milliseconds", 1e3))
            matched = next(
                (
                    (scale, name, expected)
                    for scale, name, expected in options
                    if 0.7 < rate / expected < 1.3
                ),
                None,
            )
            if matched:
                scale, name, expected = matched
                self.units[kind] = scale
                self.diagnostics[kind] = {
                    "unit": name,
                    "rate_ratio": round(rate / expected, 3),
                    "verified": True,
                }
            else:
                self.diagnostics[kind] = {
