...
class Runtime:
    def __init__(self):
        ...
        self.config = json.loads(
            (file if file.exists() else ROOT / "config.example.json").read_text(
                encoding="utf-8-sig"
            )
        )
        ...
        self.model_status = {"vision": "loading", "audio": "loading"}
        self.detection_ms = 0
        self.audio_ms = 0
        self.classification_time = 0
        self.frame_times = deque(maxlen=60)
        self.drops = 0
        self.frame_signal = asyncio.Event()
        self.recorder = Recorder(ROOT / "recordings")
        self.tasks = []
        ...
        self.rotation_sample = None
        self.seen_audio_time = None
        self.clock_samples = {}
    ...
    async def initialize(self):
        ...
        self.server = await asyncio.start_server(
            self.capture_client, "127.0.0.1", self.config["capture_port"]
        )
        self.tasks = [
    ...
    async def load_models(self):
        ...
        await asyncio.gather(
            load("vision", lambda: Detector(ROOT / "models/yolov8n.onnx")),
    ...
    def snapshot(self):
        ...
        fps = (
            ...
            if len(self.frame_times) > 1 and self.mode != "idle" and now - self.frame_time < 2
            else 0
        )
        return {
            "mode": self.mode,
            ...
            "audio_inference_ms": round(self.audio_ms, 1),
            "classification_age_ms": round((now - self.classification_time) * 1000)
            if self.classification_time
            else None,
            "native": self.native,
            "targets": self.targets if now - self.frame_time < 2 else [],
            "events": active,
            "history": history,
            "recording": self.recorder.folder.name if self.recorder.file else None,
            "audio_drops": self.drops,
    ...
    async def start(self, mode="live", session=None):
        ...
        if mode == "replay":
            safe_session(ROOT / "recordings", session or "")
        async with self.lifecycle:
            ...
            if mode == "live":
                self.runner = asyncio.create_task(self.capture_supervisor(self.generation))
            elif mode == "replay":
                self.runner = asyncio.create_task(self.replay(session, self.generation))
            else:
                self.phase = "running"
    ...
    async def capture_supervisor(self, generation):
        ...
        if not exe.exists():
            self.phase = "error"
            self.error = "未找到相机采集程序，请先运行 scripts/build.ps1"
            return
        ...
        for attempt in range(3):
            ...
            if self.config.get("software_decode"):
                args.append("software")
            env = os.environ.copy()
            env["PATH"] = str(media / "bin") + os.pathsep + env.get("PATH", "")
            try:
                ...
                while generation == self.generation:
                    ...
                    if self.process.returncode is not None:
                        raise RuntimeError(
                            f"采集进程退出 ({self.process.returncode})，详见 runtime/capture.log"
                        )
                    if (
                        self.native.get("connected_at", 0) > self.capture_started
                        and stream_start is None
                    ):
                        stream_start = now
                    if self.frame_time and now - self.frame_time < 2:
                        self.phase = "running"
                        self.error = None
                    elif (
                        stream_start and now - stream_start > 5
                    ) or now - self.capture_started > 30:
                        raise RuntimeError("相机未持续输出有效拼接帧；检查 USB、模式及 SDK 日志")
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                self.error = str(exc)
                self.phase = "retrying" if attempt < 2 else "error"
                self.fusion.clear()
                self.targets = []
                self.jpeg = None
                self.rgb = None
    ...
    async def capture_client(self, reader, writer):
        ...
        try:
            while generation == self.generation:
                ...
                if not 0 < size < 65536:
                    raise ValueError("Invalid capture header length")
                header = json.loads(await reader.readexactly(size))
                length = header["payload_bytes"]
                if not isinstance(length, int) or not 0 <= length <= 32 * 1024 * 1024:
                    raise ValueError("Invalid capture payload length")
                data = await reader.readexactly(length)
                kind = header["kind"]
                if kind == "status":
                    header["connected_at"] = time.perf_counter()
                    self.native = header
                    self.load_calibration()
                elif kind in ("rgba", "aac"):
                    self.observe_clock(kind, header)
                    if kind == "rgba":
                        self.latest_raw = (header, data, generation)
                        self.frame_signal.set()
                    else:
                        self.enqueue(self.raw_queue, (header, data, generation))
        except (asyncio.IncompleteReadError, ConnectionError):
            pass
        except Exception as exc:
            self.error = "采集协议错误: " + str(exc)
        finally:
            writer.close()
    ...
    async def frame_worker(self):
        while True:
            await self.frame_signal.wait()
            self.frame_signal.clear()
            item = self.latest_raw
            self.latest_raw = None
            if not item:
                continue
            ...
            try:
                ...
                if not 0 < w <= 4096 or not 0 < h <= 2048 or len(data) != w * h * 4:
                ...
                if generation == self.generation and epoch == self.capture_epoch:
                    self.publish_frame(rgb, jpeg, header["host_time"], header.get("camera_us"))
            except Exception as exc:
                self.error = str(exc)
    ...
    async def audio_worker(self):
        while True:
            ...
            if generation != self.generation or self.mode not in ("live", "replay"):
                continue
            ...
            self.recorder.write(
                "audio",
                pcm,
                {"timestamp": timestamp, "rate": rate, "source": source, "camera_us": camera_us},
            )
            if source == "camera" and self.channels == 4:
                self.calibration_ring.append((timestamp, pcm.copy()))
            if not self.classifier or not self.window.ready(time.perf_counter()):
                continue
    ...
    async def vision_worker(self):
        ...
        while True:
            ...
            if self.detector is None or self.rgb is None or last == self.frame_id:
                continue
            last = self.frame_id
            rgb = self.rgb
            ...
            try:
                ...
                result = await asyncio.to_thread(
                    self.detector.detect, rgb, timestamp, self.forward_offset
                )
                if generation == self.generation and epoch == self.capture_epoch:
                    self.targets = result
                    self.detection_ms = (time.perf_counter() - start) * 1000
            except Exception as exc:
                self.model_status["vision"] = "error: " + str(exc)
    ...
    async def watchdog(self):
        ...
        while True:
            ...
            now = time.perf_counter()
            if self.mode == "live":
                if now - self.frame_time > 2:
                    self.fusion.clear()
                    self.targets = []
                if (
                    self.config["microphone_fallback"]
                    and now - max(self.last_camera_audio, self.capture_started) > 5
                    and not self.mic
                    and now > mic_retry
                ):
                    try:
    ...
    async def replay(self, session, generation):
        try:
            ...
            if not entries:
                raise ValueError("录制为空")
            start = time.perf_counter()
            original = entries[0]["timestamp"]
            self.phase = "running"
            for entry in entries:
                ...
                if entry["kind"] == "frame":
                    jpeg = (folder / entry["file"]).read_bytes()
        ...
        async with self.calibration_lock:
            ...
            try:
                ...
                if (
                    ...
                    raise ValueError("采集状态改变，本次标定已取消")
                data = [
                    pcm for t, pcm in self.calibration_ring if start <= t <= time.perf_counter()
                ]
                if sum(len(p) for p in data) < self.audio_rate:
