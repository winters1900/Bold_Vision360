...
class Detector:
    ...
    def detect(self, rgb, timestamp, forward=0):
        ...
        for row in output:
            ...
            if cls not in CLASSES or score < 0.35:
                continue
            cx, cy, bw, bh = row[:4]
            cx = (cx - ox) / scale - pad
            cy = (cy - oy) / scale
            ...
            detections.append(
                {
                    ...
                    "confidence": score,
                    "bbox": bbox,
                    "angle": float(((cx / w - 0.5) * 360 - forward) % 360),
                    "timestamp": timestamp,
                }
            )
        kept = []
        for d in sorted(detections, key=lambda d: -d["confidence"]):
            if not any(
                d["label"] == k["label"] and circular_iou(d["bbox"], k["bbox"]) > 0.45 for k in kept
            ):
                kept.append(d)
        used = set()
        for d in kept:
            matches = [
                ...
                if p["track_id"] not in used
                and p["label"] == d["label"]
                and 0 < timestamp - p["timestamp"] < 0.8
                and abs(delta(p["angle"], d["angle"])) < 20
            ]
            prev = (
                min(matches, key=lambda p: abs(delta(p["angle"], d["angle"]))) if matches else None
            )
            if prev:
                d["track_id"] = prev["track_id"]
                used.add(prev["track_id"])
                dt = timestamp - prev["timestamp"]
                growth = (d["bbox"][3] / max(0.01, prev["bbox"][3]) - 1) / max(0.01, dt)
                d["growth"] = 0.7 * prev.get("growth", 0) + 0.3 * growth
                d["approaching"] = d["growth"] > 0.25
            else:
                self.track_counter += 1
                d.update(track_id=self.track_counter, growth=0, approaching=False)
        self.previous = kept
        return kept
