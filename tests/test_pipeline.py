...
def test_seam_boxes_merge():
    assert circular_iou([0.98, 0.2, 0.1, 0.2], [-0.02, 0.2, 0.1, 0.2]) > 0.99
    assert circular_iou([0.98, 0.2, 0.1, 0.2], [0.4, 0.2, 0.1, 0.2]) == 0


def test_resampling_and_discontinuity_reset():
    window = AudioWindow()
    for i in range(11):
        window.append(np.ones((4800, 2), np.float32), 48000, "camera", i * 0.1)
